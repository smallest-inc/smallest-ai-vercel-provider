...
| --- | --- |
| `lightning-v3.1` | 44.1 kHz, natural expressive speech, sub-100ms latency, 22 languages with auto-detect |
...
const { audio } = await generateSpeech({
  ...
  providerOptions: {
    smallestai: {
      ...
      enhancement: 1,            // enhancement level (0 | 1 | 2)
      addWavHeader: false,
      saveHistory: false,
...
const result = await transcribe({
  ...
  providerOptions: {
    smallestai: {
      ...
      // Streaming/WS-only knobs (forwarded for forward-compat)
      itnNormalize: true,
